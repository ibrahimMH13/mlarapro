            ...
            <div class="select-options">
                <ul>
                    @foreach($selector['options'] as $value => $option)
                        ...
                        @endphp
                        <li>
                            <a href="{{ \Ibrhaim13\Admin\Grid\Tools\Selector::url($column, $value, true) }}"
                               class="{{$active ? 'active' : ''}}">{{ $option }}</a>
                            @if(!$active && $selector['type'] == 'many')
                                &nbsp;
                                <a href="{{ \Ibrhaim13\Admin\Grid\Tools\Selector::url($column, $value) }}" class="add"><i
                    ...
                    @endforeach
                    <li>
                        <a href="{{ \Ibrhaim13\Admin\Grid\Tools\Selector::url($column) }}" class="clear"><i
